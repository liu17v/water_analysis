...
import { Loading, WarningFilled, Document, Download, FullScreen, Operation, Edit } from '@element-plus/icons-vue'
...
async function saveReservoirName() {
  try {
    await api.updateTask(task.value.task_id, { reservoir_name: editReservoirName.value || '' })
...
async function loadFullTaskInfo() {
  try {
    const res = await api.getTasks(1, 1, { search: route.params.id })
    const full = (res.items || []).find(t => t.task_id === route.params.id)
    if (full && task.value) {
      task.value.reservoir_name = full.reservoir_name
      task.value.original_filename = full.original_filename
    }
  } catch {}
}
