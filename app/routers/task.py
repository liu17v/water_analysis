...
from app.services.visualization import INDICATOR_LABELS
from app.services.milvus_service import delete as milvus_delete
from app.config.logging import get_logger

logger = get_logger("routers.task")
...
async def task_list(
    ...
    status: str = Query(None, description="按状态筛选: pending/processing/success/failed"),
    search: str = Query(None, description="搜索任务ID/水库名称/文件名"),
    start_date: str = Query(None, description="开始日期 YYYY-MM-DD"),
    end_date: str = Query(None, description="结束日期 YYYY-MM-DD"),
    sort: str = Query(None, description="排序字段: total_points/anomaly_count/created_at"),
    ...
    if search:
        pattern = f"%{search}%"
        search_filter = (Task.id.ilike(pattern)) | (Task.reservoir_name.ilike(pattern)) | (Task.original_filename.ilike(pattern))
        base_q = base_q.where(search_filter)
        count_q = count_q.where(search_filter)

    if start_date:
        base_q = base_q.where(Task.created_at >= start_date)
        count_q = count_q.where(Task.created_at >= start_date)
    if end_date:
        base_q = base_q.where(Task.created_at <= f"{end_date} 23:59:59")
        count_q = count_q.where(Task.created_at <= f"{end_date} 23:59:59")

    result = await db.execute(count_q)
    total = result.scalar()

    sort_col = getattr(Task, sort, None) if sort and sort in ("total_points", "anomaly_count", "created_at") else Task.created_at
    if order == "asc":
...
async def get_visualization(
    ...
    grids = await loop.run_in_executor(None, interpolate_all_layers, rows, indicator)
...
async def get_contour_html(
    ...
    html = await loop.run_in_executor(None, gen_contour, grid, indicator, d, anomaly_points)
...
async def get_statistics(task_id: str, db: AsyncSession = Depends(get_db)):
    ...
    for col in INDICATOR_COLS:
        ...
        short_code = INDICATOR_SHORT_MAP.get(col, col)
        col_anomaly_count = await db.execute(
            select(func.count()).select_from(AnomalyRecord).where(
                AnomalyRecord.task_id == task_id, AnomalyRecord.indicator == short_code
            )
        )
...
async def process_task(task_id: str, db: AsyncSession = Depends(get_db)):
    ...
    bg_task = asyncio.create_task(run_task_local(task_id))
...
async def delete_task(task_id: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, milvus_delete, task_id)
    except Exception:
        pass
