    ...
    <div v-else class="card-grid">
      ...
      <template v-else>
        <el-card v-for="t in tasks" :key="t.task_id" shadow="hover" class="task-card"
          ...
          <div class="card-name">{{ t.reservoir_name || '未知水库' }}</div>
...
import { Plus, Search, Refresh, List, Grid, Delete } from '@element-plus/icons-vue'
...
async function fetchData() {
  ...
  try {
    ...
    if (sortField.value) {
      ...
      params.order = sortOrder.value || 'desc'
    }
    const res = await api.getTasks(page.value, pageSize.value)
...
async function onDelete(taskId) {
  try { await api.deleteTask(taskId); fetchData() } catch {}
}
